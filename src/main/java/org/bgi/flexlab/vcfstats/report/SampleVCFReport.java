...
public class SampleVCFReport {
    ...
    private static final int ALLELE_LENGTH_MAX_PRINT = 50;
    ...
    private long mGP08 = 0;
    private long mGP09 = 0;
    ...
    public SampleVCFReport(boolean countVarLength){
        sampleName = null;
...
//            return;
        ...
        if(gt.hasExtendedAttribute("GP")){
            ...
            float maxGP = StatsUtils.MAX(GP.split(","));
            if(maxGP >= 0.8){
                mGP08 += 1;
            }
...
//        values.add(StatsUtils.divide(mHeterozygousMnps, mHomozygousMnps));
        ...
        names.add("max_GP >=0.8 ratio");
        values.add(StatsUtils.divide(mGP08, mTotalPassedFilters - mNoCall - mTotalUnchanged));
        names.add("max_GP >=0.9 ratio");
        values.add(StatsUtils.divide(mGP09, mTotalPassedFilters - mNoCall - mTotalUnchanged));
